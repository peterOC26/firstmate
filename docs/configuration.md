...
An Orca task additionally records `orca_worktree_id=` and `terminal=`, with `window=fm-<id>` kept as the shared firstmate alias; a task targeted through an `orca:` project selector also records `orca_host=` and `orca_project_host_setup=`, and one placed on a remote Orca host also records `orca_remote=1` and `orca_remote_tasktmp=` (see [`docs/orca-backend.md`](orca-backend.md)).
