...
Pi 0.81.1 was installed when Calm was first built, Pi 0.82.0 was the first reverification target, Pi 0.84.1 was the SuperCalm tool-row target, and Pi 0.84.2 is the latest reverification target.
...
The fix installs one idempotent presentation adapter on the exported `AssistantMessageComponent.updateContent` method, over the verified Pi versions recorded above.
...
Tool images remain visible because Pi appends image children outside the hidden text shell, and Calm preserves that parity for built-in and foreign rows.
...
The fix installs a separate idempotent presentation adapter on the exported `InteractiveMode.addMessageToChat` method, over the verified Pi versions recorded above.
The adapter probes for that exact method and, per the [compatibility contract](calm.md#pi-compatibility), degrades independently with a diagnostic rather than gating on a version number.
...
| `tool-image` | Image children appended outside tool renderer slots | Remains visible for built-in and foreign rows by parity with the original Calm built-in behavior; the hidden row still renders its image children (verified on Pi 0.84.1 and 0.84.2). |
...
The installed extension API has no supported global transcript filter, user-message renderer, assistant-message renderer, chat-container API, or tool-registration wrapper that can safely claim arbitrary foreign definitions.
Pi 0.81.1 through 0.84.2 and Pi 0.84.4 export `AssistantMessageComponent`, `ToolExecutionComponent`, and `InteractiveMode`, so Calm uses separate idempotent, API-probed adapters for assistant thinking layout, generic tool-row layout, and the complete operational-user transcript row while leaving all message data and non-Calm rendering unchanged; see the [compatibility contract](calm.md#pi-compatibility) for how a future Pi lacking one of those exports is handled.
...
It leaves the export-return reset itself unchanged; the 2026-08-15 export-confirmation verification below owns why that reset repaints only the rows Calm presents and requests the render through a cleared extension footer status rather than a tool-expansion cycle.
All three render-time adapters - the tool row, the operational-user row, and the assistant-message row that collapses thinking and mid-turn working notes - read the Calm policy without the stock-export escape hatch, because Pi builds `/export` and `/share` output from tool definitions and session entries rather than from on-screen rows: honoring the hatch there would only unhide those rows for the single frame Pi paints inside the export window.
...
The tool-definition wrappers still honor it, and they are what keeps exported and shared artifacts complete: `AgentSession.exportToHtml` hands the exporter a `createToolHtmlRenderer` plus the session entries, so those wrappers are the only Calm seam the export path actually calls.
Registered entry renderers are screen-only as well - `getEntryRenderer` has a single call site, the on-screen transcript in `InteractiveMode`, and custom entries reach exported HTML through the exporter's own generic `customType` block instead - so the escape hatch the synthetic entry renderer reads is inert rather than load-bearing.
It is left in place because no reachable sequence was found in which it strands a row on screen: a custom entry re-runs its renderer only from its constructor, `setExpanded` (Ctrl+O), or `invalidate()`, and none of the whole-transcript invalidate paths are reachable inside the sub-tick export window.
...
| Pi (verified 0.81.1 through 0.84.2) | Partially feasible with three API-probed exported-class adapters. | Public APIs control working visibility, collapsed labels, known tool slots, custom entries, and expansion redraws; exported assistant, tool-execution, and interactive-mode classes provide the collapsed-thinking, generic tool-row, and operational-user layout boundaries, gated on the exact method's presence rather than a version number, while generic user, non-tool custom-entry, and status filtering remains unavailable. |
...
`tests/fm-calm-pi-extension.test.sh` compares wrapped and stock renderers, verifies all seven built-ins plus `fm_watch_arm_pi`, foreign registered tools, unregistered provider-side tool rows, contested built-in rows, and image parity; `tests/fm-pi-branch-extension.test.sh` verifies `fm_branch_outcomes` Calm toggling, capability-probed all-line versus collapsed stock output, exact expanded output, and export rendering.
...
`tests/fm-pi-primary-types.test.sh` performs strict no-emit TypeScript checking against the installed Pi declarations and reports the declaration package version it used, rather than pinning one here.
