...
test_remote_pr_discovery_names_the_task_s_own_forge_host() {
  ...
  git -C "$PROJ" remote set-url origin https://github.acme.invalid/team/app.git
  printf 'hello\n' > "$WT/feature.txt"
  git -C "$WT" add feature.txt
  git -C "$WT" -c user.email=t@t -c user.name=t commit -qm "add feature"
...
test_remote_pr_discovery_keeps_a_forge_port() {
  ...
  git -C "$PROJ" remote set-url origin https://git.acme.invalid:8443/team/app.git
  printf 'hello\n' > "$WT/feature.txt"
  git -C "$WT" add feature.txt
  git -C "$WT" -c user.email=t@t -c user.name=t commit -qm "add feature"
  head=$(git -C "$WT" rev-parse HEAD)
...
test_remote_pr_discovery_drops_an_ssh_transport_port() {
  ...
  git -C "$PROJ" remote set-url origin ssh://git@git.acme.invalid:2222/team/app.git
  printf 'hello\n' > "$WT/feature.txt"
  git -C "$WT" add feature.txt
  git -C "$WT" -c user.email=t@t -c user.name=t commit -qm "add feature"
  head=$(git -C "$WT" rev-parse HEAD)
...
test_remote_pr_discovery_refuses_to_guess_a_host_from_an_ssh_alias() {
  ...
  git -C "$PROJ" remote set-url origin 'git@github-work:team/app.git'
...
test_spawn_writes_orca_metadata_and_launches_harness() {
  ...
  assert_contains "$(cat "$log")" "CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false claude --session-id '" \
    "spawn did not send the selected harness launch command through Orca"
  assert_contains "$(cat "$log")" "--dangerously-skip-permissions" \
    "spawn did not preserve the Claude permission flag through Orca"
  rm -rf "/tmp/fm-$id"
