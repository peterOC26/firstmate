...
#
# THE CONFIGURED DONE ARCHIVE IS A LOOKUP FALLBACK, NEVER A WEAKER GATE.
# Every `tasks-axi done` prunes past `done_keep`, so a genuinely answered call
# can leave the live backlog entirely for the configured Done archive (the
# `[markdown] archive` value read from `.tasks.toml`, then the user config,
# then the default beside the backlog). `verify`, the `complete` gate it
# shares, and `answer`'s already-closed record path therefore fall back to that
# archive. The live record still decides alone whenever the backlog holds the
# identity, and a live read that fails for anything other than a genuine
# NOT_FOUND refuses rather than consulting an older archived copy. An archived
# record is never read as actively held, so it passes only as a closed call
# carrying the same durable resolution block; an identity absent from both
# still refuses. Ids are deterministic and the archive is append-only, so one
# identity can appear there more than once: the occurrence in the newest
# `## Archived` block is the current record, and two occurrences inside one
# block are reported as unresolvable ambiguity rather than settled by an
# arbitrary pick. Recording an answer on an archived call rewrites only that
# newest occurrence, in the archive file itself, under tasks-axi's own advisory
# backlog lock, because a prune's archive append and backlog rewrite happen
# inside that same lock; a lock this cannot take fails closed, reporting that
# the decision was not recorded and leaving the archive untouched.
#
...
# Does this identity have a record at all? The live backlog answers first; a
# call answered and then pruned past `done_keep` still exists, in the configured
# Done archive. An identity the archive cannot reduce to one newest record also
# exists - reporting it as absent would point the operator at the one recovery
# that cannot work - so the precise refusal is left to the reader that follows.
task_record_exists() {  # <id>
  local archive rc=0
  task_show "$1" >/dev/null 2>&1 && return 0
  archive=$(tasks_axi_archive_path 2>/dev/null) || return 1
  [ -f "$archive" ] || return 1
  archive_task_show "$archive" "$1" >/dev/null 2>&1 || rc=$?
  [ "$rc" -eq 0 ] || [ "$rc" -eq "$ARCHIVE_RECORD_AMBIGUOUS_RC" ]
}
...
fail_unresolvable_archive_record() {  # <id> <archive>
  fail "captain-held task $1 has $TASK_RECORD_AMBIGUOUS occurrences in the configured archive $2 with no single newest one; leave one current record for $1 there before this gate can read it"
}

fail_task_record_lookup() {  # <id>
  local id=$1 archive
  [ -z "$TASK_RECORD_LIVE_ERROR" ] \
    || fail "could not read captain-held task $id from the active backlog: $TASK_RECORD_LIVE_ERROR"
  ...
  fail "captain-held task $id is absent from the active backlog and configured archive $archive"
}
...
verify_hold_durable() {  # <task-id>
  ...
  # The live backlog decides alone whenever it still holds the identity; a
  # genuinely answered call that a `tasks-axi done` prune moved into the
  # configured Done archive is read from there instead of being reported absent.
...
resolve_entry() {  # <origin-or-empty> <entry>; prints the resolved id or fails
  ...
  if task_record_exists "$entry"; then
    printf '%s' "$entry"
    return 0
  fi
  archive=$(tasks_axi_archive_path 2>/dev/null || printf '<unavailable>')
  if [ -n "$origin" ] && [ "$origin" != "$BINDING_ANY" ]; then
    ...
    if task_record_exists "$legacy"; then
      printf '%s' "$legacy"
      return 0
    fi
    fail "captain-held task $entry is absent from the active backlog and configured archive $archive, and so is the legacy identity $legacy"
  fi
  fail "captain-held task $entry is absent from the active backlog and configured archive $archive"
}
...
write_resolution_record() {  # <task-id> <mode> <shown-body> [<archive-file>]
  local id=$1 mode=$2 body=$3 archive=${4:-} new_body tmp
  ...
  if [ -n "$archive" ]; then
    # The record lives in the configured Done archive, so the rewrite happens
    # there, under tasks-axi's own advisory backlog lock. tasks-axi's
    # --archive-body pristine copy has no archived equivalent, so the archived
    # body is replaced by the block plus the body it already carried.
    archive_task_update_body "$archive" "$id" "$new_body" \
      || fail "could not record the captain decision on archived task $id"
    return 0
  fi
...
command_answer() {
  local id=${1:-} decision_file='' release=0 show state hold_kind body outcome recorded_mode record_file
  [ "$#" -ge 1 ] || { usage >&2; exit 2; }
  shift
  while [ "$#" -gt 0 ]; do
    case "$1" in
  ...
  if [ "$state" = "done" ]; then
    ...
    write_resolution_record "$id" repaired "$body" "$record_file"
  ...
  [ -z "$record_file" ] \
    || fail "captain-held task $id is archived and not closed; restore it to the active backlog first"
...
command_complete() {
  ...
  if [ -n "$keys" ]; then
    while IFS= read -r entry; do
      [ -n "$entry" ] || continue
      # resolve_entry's own refusal runs in this substitution's subshell, so its
      # status has to be carried out by hand or an empty id reaches the gate.
