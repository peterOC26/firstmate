...
Reverified on 2026-09-20 on Linux with Bash 5.2, jq 1.7, and repository-pinned ShellCheck 0.11.0.
That run reported `board-sync tests: 38 passed`, before the subsequent projection and issue-state regressions.
The additional regression passes fleet and bearings snapshots larger than 200 KB through the dry-run interface and verifies the resulting issue plan, protecting stdin routing against Linux's single-argument size limit.
...
The original behavioral cases cover custom-check arm, status, and disarm with trust-binding verification, withdrawal of a check whose trust binding fails, allowlisted issue bodies that reject credential-bearing artifacts, exclusion-file enforcement in both reconcile and poll, opaque titles for tasks with no structured title, identity-owned single-instance reconcile locking that blocks a second run while a live owner holds the lock, serializes competing stale-owner reclaimers, reclaims dead ones, and cannot be wedged by an interrupted claim publication, private-repository refusal at startup and again immediately before mutation, mutation-free dry runs for both new and already-mapped tasks, a dry-run plan that matches the real operation list field for field, persisted state that holds only the task mapping, pointer-only poll deduplication, a card off its fleet column that is noted and pushed back, a cleared Status that is noted and restored, a card removed from the board that is noted and restored at its fleet column, an archived card that is reported and never written, unarchived, or deleted, a closed issue that is reported and never reopened, an excluded task that receives no card, no write, and no note, an unmanaged card that is reported and left untouched, a separate canonical card created beside a same-title hand-filed issue and beside a manual draft, a card re-added under a new board item id that reconciles and rebinds instead of wedging, issue writes that follow the repository a task's own mapping records rather than a reconfigured one, a mapping repository made public between its pre-write confirmation and the write itself that is refused at the mutation boundary, an unreachable mapping repository that skips only its own task while the rest of the run still pushes and still records its post-run state, a board read that survives an all-digit GitHub login, an already-reported board difference that stops re-waking the watcher while a new one still wakes it, board moves, archives, issue closes, and unmanaged-card changes landing inside the reconcile window that are never silently absorbed, a bare GitHub touch that never re-wakes the poll, and notes that report only what the run actually observes.

Current regression entry points also include `test_return_catchup_warning_never_creates_a_task_card`, `test_cached_contribution_wins_duplicate_task_rows`, `test_pr_enrichment_preserves_task_actionability`, and `test_live_discovery_preserves_cached_contribution_identity`.
`test_missing_card_checks_issue_state_before_restoration` covers closed, open, Done, and unavailable-state restoration paths; `test_closed_issue_is_noted_and_never_reopened` includes mismatched issue and card fields.
These are suite coverage pointers, not additional live GitHub verification claims.
...
Seven guarantees were confirmed to fail when their implementation was neutralized on this revision, so they do not pass vacuously.
...
Sending issue writes to the configured repository instead of the one a task's mapping records failed `issue writes must target the repository the mapping records (missing: 'repos/captain/legacy/issues/1')`, and that same neutralization is what proves the mutation-boundary gate is read against the mapping's own repository rather than the configured one.
Removing `github_mutation_guard` from `issue_update` and `issue_close` failed `a mapping repository made public before mutation must refuse the write`.
Turning the per-task skip for an unconfirmable mapping repository back into a whole-run refusal failed `one unreachable mapping repository must not abort the whole reconcile`.

The original 2026-08-18 focused result was:

```text
board-sync tests: 37 passed
```
