...
# Orca owns worktree creation and freshness, including remote paths unavailable locally.
...
if [ "$RELAUNCH" -ne 1 ]; then
  # Publish through a sibling temp file so readers see either no record or the
  # complete one.
  META_TMP=$(mktemp "$STATE/.$ID.meta.XXXXXX") || {
    echo "error: could not publish task metadata: $STATE/$ID.meta" >&2
    exit 1
  }
  SPAWN_META_PATH=$META_TMP
fi
{
...
if [ "$RELAUNCH" -ne 1 ]; then
  # Do not replace an existing non-regular metadata destination with the
  # atomic rename.
