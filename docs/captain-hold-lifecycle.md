...
Durable verification and the retroactive `answer` record read one place the other subcommands do not.
Every `tasks-axi done` prunes past `done_keep`, so a genuinely answered captain call can leave the live backlog entirely for the configured Done archive (the `[markdown] archive` value in "Backlog backend" of [`configuration.md`](configuration.md#backlog-backend-taskstoml--configbacklog-backend)), which is why `verify`, the `complete` gate it shares, and `answer`'s already-closed path fall back to that archive.
...
The fallback never weakens the gate: an archived record is never read as actively held, so it passes only as a closed call carrying the same durable resolution block, and an identity absent from both live backlog and archive still refuses while naming both locations.
Ids are deterministic and the archive is append-only, so one identity can appear there more than once; the occurrence in the newest `## Archived` block is the current record, and two occurrences inside one block are reported as unresolvable ambiguity rather than settled by an arbitrary pick.
Recording an answer on an archived call rewrites only that newest record's body, in the archive file itself, under tasks-axi's own advisory backlog lock, because a prune's archive append and backlog rewrite happen inside that same lock and an unlocked whole-file rewrite here would silently drop concurrent archived history.
A lock it cannot take fails closed: the decision is reported as not recorded and the archive is left untouched.
`bin/fm-captain-hold.sh --help` owns the exact mechanics.
...
The Done column excludes a record that closed while still held for the captain (surviving `hold-kind: captain` on a Done row), so answered questions do not masquerade as shipped work; a work item released before completion keeps no hold annotations and lands normally.
...
Nine further cases in the same suite cover the configured Done archive as a lookup fallback, carried over from the retired `tests/fm-decision-hold-lifecycle.test.sh` and driven through the retained shim, in a home whose `[markdown] archive` is deliberately not the default path.
They prove: a resolved call pruned out of the live Done window still satisfies `verify` and teardown from the archive; an archived call closed without a durable captain answer refuses both, keeps its task metadata, and is then recorded in place so the same origin clears; an archived unanswered open call is never read as actively held and refuses both verification and teardown; a call absent from both locations refuses and names both; an identity present in both is decided by the live record alone; a live read that fails for anything other than a genuine not-found never falls back; duplicated archived occurrences are decided by the newest `## Archived` block, rewritten in that same occurrence, and reported as ambiguity when one block holds two; archive rewrites take tasks-axi's own backlog lock, including the one named by `TASKS_AXI_FILE`; column-zero content outside task bodies survives both archive parsing and archive rewriting; and the completion attestation serializes on the shared task metadata lock.
...
Projection regressions live in `tests/fm-fleet-snapshot-view.test.sh` (hold-until parsing, the due gate, kind-independent captain actionability, deferred_marker, title stripping) and `tests/fm-bearings-snapshot.test.sh` (Waiting on you membership, the dated-gate rendering, prose-deferral suppression with disclosure, and the landed exclusion by surviving captain-hold annotations).
