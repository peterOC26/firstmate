...
When another worktree already holds that branch, git's refusal stands unless the holder is provably abandoned: a registration whose directory is gone, or a copy that is neither the primary checkout nor the project checkout, whose task record, when one names it, reads positively agent-free, and that holds no live process.
Every other holder, including one whose liveness cannot be told, is refused, so a leaked pool slot stays recoverable while a second live copy never shares the branch; the shared `lsof` proof recursively checks holder directories and their descendants.
