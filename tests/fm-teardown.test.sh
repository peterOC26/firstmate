...
test_teardown_closes_the_backlog_item_itself() {
  ...
  printf '%s\n' "$out" | grep -F 'keep Done to the 30 most recent' >/dev/null \
  ...
  pass "teardown closes its own backlog item and prompts tasks-axi refresh when compatible"
}
...
test_teardown_manual_backend_leaves_the_backlog_to_the_operator() {
  ...
  printf '%s\n' "$out" | grep -F 'keep Done to the 30 most recent' >/dev/null \
    || fail "teardown manual prompt does not match done_keep = 30: $out"
  printf '%s\n' "$out" | grep -F 'tasks-axi done' >/dev/null \
    && fail "teardown prompted tasks-axi despite manual backend opt-out: $out"
  pass "teardown honors config/backlog-backend=manual even when tasks-axi is compatible, and still finishes cleanly"
}
