...
`docs/configuration.md` is the single owner of the top-level operational-home layout and every `config/` schema; each producing script's header and help own exact child fields and mutation mechanics.
Read those owners before relying on a file below, because this section keeps only what applies at every session.
...
`data/` holds durable private fleet records: `backlog.md` (section 10), the `projects.md` fleet navigation registry recording each project's standing delivery posture, the `secondmates.md` local and remote routing table, `captain.md`, optional `captain-shared.md`, `learnings.md`, each task's `<id>/brief.md`, and the scout deliverable `<id>/report.md`, which survives teardown.
`state/` holds volatile runtime records: each task's `<id>.meta` written by `bin/fm-spawn.sh`, the append-only `<id>.status` event log, the authenticated `<id>.check.sh` slow poll, the durable wake queue, the `.afk` away-mode flag, generated Relay artifacts (section 14), registered process-to-event sources, and the coordination records each writing script owns.
`config/` holds local gitignored operating choices, and `projects/` contains clones that are read-only to firstmate except under hard rule 1's concrete captain-approved project operation exception.

A `state/<id>.status` line is a wake event, not current-state truth; `bin/fm-crew-state.sh` owns current-state reconciliation.
Treat `data/captain.md` as the domain-local record of captain preferences, optional `data/captain-shared.md` as the main-authoritative shared captain-preference file for secondmate inheritance, and `data/learnings.md` as curated home-local knowledge, regardless of harness memory.
Never hand-edit or delete the watcher, sub-supervisor, Claude auto-arm, lock, migration, quarantine, or PR-poll records under `state/`; the scripts that write them own their lifecycle, and teardown removes a finished task's own records.
...
The digest's drained wake records are this turn's first work queue, and its `OPEN DECISIONS` section is actionable reconciliation input even when no wake was queued.
The digest never starts supervision itself; section 8 and the emitted block own that.

If the session lock cannot be acquired and verified, report its exact diagnostic and remain read-only; another active session is only one possible cause.
A lock-refused session must not spawn, steer, merge, drain the wake queue, repair supervision, repair a checkout, or perform any other fleet mutation, and it runs no network checks at all.

The digest itself makes no external-network call and never waits for one; every network check a session start owes - GitHub auth, dead-secondmate relaunch, secondmate convergence, pending handoff delivery, and project clone refresh - runs in the deferred stage owned by `bin/fm-startup-network.sh` and is reported in the digest's own `NETWORK CHECKS` section.
When that section reports checks still in progress it names exactly what is unconfirmed; treat none of those as passed until the result lands, either from `bin/fm-startup-network.sh report` or as a `check: startup-network` wake.
...
Worker context usage is measurement-only: `unknown` is first-class, and no result changes worker lifecycle.
Load `context-usage` when auditing worker context usage or handling a context usage warning or unknown result.
...
Firstmate alone resolves a matched profile array: load `quota-array-dispatch`, the single owner of that completion-aware selection procedure, before choosing, and run `quota-axi --json` once at that intake.
Account for every configured candidate against that current output - its catalog evidence, provider relationship, applicable quota and authentication facts, remaining uncertainty, fit and reasoning class, and the headroom, runway, and later pace or reserve evidence used in selection; never omit a candidate, guess, fall back silently, or call the result quota-informed without them.
Missing model-level quota, a missing authentication source, unmeasurable headroom, or unmodeled authentication is disclosed uncertainty that keeps a candidate eligible, never a credential or login escalation; only concrete contradictory evidence blocks a candidate.
...
Corrections required to satisfy already accepted intent are not new requirements: the smallest downstream changes that keep already accepted product or engineering behavior correct, add behavioral tests where an executable contract exists, or keep documentation accurate remain within the current task even when they touch files not named at intake.
...
Run `bin/fm-pr-check.sh <id> <PR url>` - it records the PR in the task's meta and arms the watcher's merge poll.
...
When implementation is separately authorized, promote the existing scout through `bin/fm-promote.sh` rather than creating a duplicate task, and send the promoted worker the ship instructions its header names.
The promoted worker leaves scratch commits and debug edits behind and turns a reproduced bug into the regression test.
...
Relay opt-in and any registered process-to-event source require that same live cycle with no fleet work.
...
- `context-usage` - load before auditing dispatched-worker context usage, and on any context usage warning or unknown result.
