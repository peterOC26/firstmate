...
# Metadata writers share the task-metadata lock with other producers. Load the
# lock owner only when a metadata writer is called: sourcing fm-wake-lib.sh at
# library load time would create $FM_HOME/state during unrelated X-mode reads.
fmx_meta_lock_load() {
  if ! declare -F fm_meta_lock_path >/dev/null 2>&1; then
    # shellcheck source=bin/fm-wake-lib.sh
    . "$_FM_X_LIB_DIR/fm-wake-lib.sh"
  fi
}
...
fmx_meta_link_set() {
  local meta=$1 rid=$2 ts=$3 followups=${4:-0} platform=${5:-} reply_max=${6:-} tmp lock rc=0
  [ -f "$meta" ] || return 1
  fmx_meta_lock_load
  ...
  if [ "$rc" -eq 0 ]; then
    ...
    fm_backlog_atomic_transition publish "$tmp" "$meta" "task record" "$STATE" || rc=1
  fi
  [ "$rc" -eq 0 ] || rm -f "$tmp"
...
fmx_meta_followups_set() {
  local meta=$1 n=$2 tmp lock rc=0
  [ -f "$meta" ] || return 1
  fmx_meta_lock_load
  lock=$(fm_meta_lock_path "$meta") || return 1
  fm_lock_acquire_wait "$lock"
