...
# A terminal stale state is already delivered when this task's status log
# carries no actionable record past the position already surfaced to firstmate.
# The position, not the last line, is the one owner of "already surfaced"
# (fm-push-transition-lib.sh's hb_surfaced_offset), so this also stays correct
# when a later routine append moves past the terminal line - the case a last-line
# comparison cannot see. Anything other than a clean "nothing new" answer,
# including an unreadable log, surfaces rather than absorbs.
stale_state_already_delivered() {  # <task> <last-status-line>
  local task=$1 last=$2 record rc
  [ -n "$task" ] || return 1
  [ -n "$last" ] || return 1
  record=$(status_span_first_actionable_record \
    "$STATE/$task.status" "$(hb_surfaced_offset "$task")")
  rc=$?
  : "$record"
  # 0 = an actionable record sits past the surfaced position (surface it);
  # 1 = nothing new there (already delivered); 2 = unreadable, so surface.
  [ "$rc" -eq 1 ]
}
...
while :; do
  ...
  if [ -n "$pending" ]; then
    ...
    context_reason=
    context_record=''; context_end=''; context_ident=''
