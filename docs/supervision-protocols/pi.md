...
Treat the originating wake as already handled: MAIN must not re-drain, re-run, or acknowledge it.
A pending continuation handed off under the branch prompt's verdict rules still needs MAIN to act before acknowledging the outcome; reconcile existing actions before spawning or steering so retries do not duplicate work.
