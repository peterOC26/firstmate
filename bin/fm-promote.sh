...
3. Preserve anything you may still want before touching history: if \`git status\` shows uncommitted scratch edits, commit them (\`git add -A && git commit -m scratch\`), then note the scratch tip (\`git rev-parse HEAD\`) so you can cherry-pick or diff from it in step 4 (only the reflog keeps those commits alive once the branch moves). Make sure you are on your branch \`fm/$ID\` (a no-op if fm-spawn already created it): \`git checkout fm/$ID 2>/dev/null || git checkout -b fm/$ID\`. Only once \`git status\` is clean, return that branch to the default-branch base, exactly as written: \`git fetch origin && git reset --hard "\$(git symbolic-ref --short refs/remotes/origin/HEAD)"\` (that symbolic-ref already prints the remote-tracking ref, such as origin/main, so it needs no origin/ prefix of its own).
