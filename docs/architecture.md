...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, except when no actionable status event remains past the successfully surfaced position, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
Pane hashes are liveness evidence, while the classified byte position in `state/.hb-surfaced-<task>` is lifecycle delivery evidence for terminal stale deduplication.
So a stale pane whose crew is not provably working is absorbed only when its status log has no actionable record beyond that successfully surfaced position: the watcher advances stale hash bookkeeping and clears that key's stale and wedge timers instead of queuing a wake.
A later routine append cannot hide an earlier surfaced transition, while any new blocked, failed, keyed-decision, PR-ready, or merged event beyond the position still surfaces in source order.
An unreadable log or unusable marker fails toward surfacing, and the shared presentation-marker contract described below keeps reported file state independent from successfully classified position.
...
An Orca task can also be placed on a remote Orca host through an explicit `orca:` project selector, in which case that host owns the worktree and runs the agent process, and the spawn-time isolation assertion and the teardown work-protection checks run there rather than on this machine.
...
`fm-spawn.sh` also owns the base-freshness boundary for every fresh treehouse-provided ship and scout: no worker starts until its clean task worktree matches the fetched tip of origin's resolved default branch, and any unsafe or unverifiable base stops the spawn.
Its header owns the exact refusal mechanics, while `tests/fm-spawn-pool-base-freshen.test.sh` owns the portable regression coverage.
Every `backend=orca` ship and scout spawn is outside that boundary, on this machine as well as on a remote Orca host: Orca creates the worktree and owns its freshness, and on a remote host the worktree is not here for firstmate to fetch or reset at all.
