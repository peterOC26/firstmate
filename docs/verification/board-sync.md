...
Its forty-one behavioral cases prove custom-check arm/status/disarm with trust-binding verification, withdrawal of a check whose trust binding fails, allowlisted issue bodies that reject credential-bearing artifacts, exclusion-file enforcement, opaque titles for tasks with no structured title, escalation of excluded tasks that still hold a card, retirement of that report once the card is gone, identity-owned single-instance reconcile locking that preserves live owners, serializes competing stale-owner reclaimers, reclaims dead ones, and cannot be wedged by an interrupted claim publication, pending-create recovery, private-repository refusal at startup and again immediately before mutation, mutation-free dry runs for both new and already-mapped tasks, pointer-only poll deduplication, escalation-only pull, wake quiescence for an already-reported board move, no silent suppression of a mapped board move that lands inside the reconcile window, distinct signatures for opposite archive transitions, no silent absorption of a foreign-card change or addition that lands in that same window, no silent absorption of a mapped-card archive or issue close that lands in that same window, explained fleet-authoritative conflict snapback, escalation of a write over an unrecorded baseline without inventing a captain move, escalation of a cleared Status by the run that restores the column, one-time escalation of an off-column hand-archived card that supersedes simultaneous move and close details while otherwise leaving the card completely alone, an ordinary forward column write that is never called a snapback, a manual board card reported as captain intent without mutation, creation of a separate canonical card when a same-title captain-authored issue carries the task token, creation of a separate canonical card for a manual draft, exactly one truthful escalation for a card removed from the board, retirement of a mapping the sync no longer owns once its card is gone, an archived card that counts pending once and wakes again only when it is unarchived, a card re-added under a new board item id that reconciles and rebinds instead of wedging, a dry-run plan that matches the real operation list field for field, exact-shape token recovery that retains the resolved live column without redundant operations, a board read that survives an all-digit GitHub login, a foreign card that counts once and then only when it really moves, a bare GitHub touch that never re-wakes the poll, and escalations that report only what the run actually observes.
...
The cases for manual issues and drafts prove that captain-authored cards remain untouched while each fleet task receives a separate canonical Firstmate-managed card.
The same-title token case proves recovery rejects a captain-authored body that does not exactly match the generated allowlisted shape, while the existing exposure negative controls prove a Firstmate-created body carries nothing beyond the allowlist.
The archive-window and close-window cases prove that post-write board reads cannot absorb captain changes the run did not report.
