...
# Record <task>'s status log as already surfaced through its current end, in the
# presentation-marker format fm-classify-lib.sh owns (a classified byte position
# plus file identity), which is what the watcher itself commits via mark_surfaced.
prime_hb_surfaced() {  # <state> <task> <status-file>
  local state=$1 task=$2 file=$3 ident
  ident=$(_fm_open_decisions_file_ident "$file") || return 1
  status_presentation_marker_commit "$state/.hb-surfaced-$task" "$file" \
    "$(size_of "$file")" "$ident"
}
...
test_delivered_terminal_footer_change_is_absorbed() {
  ...
  local raw_capture raw_status raw_window raw_key raw_hash sig absorbed blocked_end
  ...
  prime_hb_surfaced "$state" footer "$status_file" \
    || fail "could not prime the delivered terminal marker"
  key=$(printf '%s' "$window" | tr ':/.' '___')
  ...
  [ "$(status_presentation_marker_offset "$state/.hb-surfaced-raw" "$raw_status")" -gt 0 ] \
    || fail "the first terminal wake did not record delivery"
  ack_stopped_cycle "$state" || fail "could not acknowledge the first raw terminal wake"

  printf 'blocked: credentials expired\n' >> "$raw_status"
  blocked_end=$(size_of "$raw_status")
  ...
  [ "$(status_presentation_marker_offset "$state/.hb-surfaced-raw" "$raw_status")" -ge "$blocked_end" ] \
    || fail "the blocked transition did not record delivery"
  ...
  [ "$(status_presentation_marker_offset "$state/.hb-surfaced-raw" "$raw_status")" = "$(size_of "$raw_status")" ] \
    || fail "the keyed decision transition did not record delivery"
