...
When the captain says "later", that is an answer too: re-hold with `tasks-axi hold <id> ... --until <date>` so the item leaves the live Waiting on you column and resurfaces on its date, instead of leaving a live-looking card or fabricating a closure.
...
5. Relay the choices to the captain as decisions from Bearings' Waiting on you column under `AGENTS.md` section 9; do not use the word hold in captain chat.
...
7. Confirm Bearings reflects the outcome: answered calls leave Waiting on you, released work resumes, and deferred calls sit in Held with their date.
   The interactive `/bearings lavish` board keeps its own section names; `.agents/skills/bearings/SKILL.md` owns both surfaces.
