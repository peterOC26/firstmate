...
The promoted worker must inventory and preserve wanted scratch state, confirm its `fm/<id>` branch, reset that branch to a clean default-branch base, carry over only intended fix changes, and follow the project's selected delivery path while leaving scratch commits and debug edits behind and turning a reproduced bug into the regression test.
